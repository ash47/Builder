...
plugin.get('LobbyManager', function(obj){
	// Store the lobby manager
	gpUlt = obj.getOptionsForPlugin;

	// Default gold per second to 0
	var goldPerSecond = 0;

	// Grab options
	var options = obj.getOptionsForPlugin("Builder1");
	if(!options) return;

	// Grab options
	var gps = options["GoldPerSecond"];

	// Ensure we can find the lobby manager
	if(gps) {
		// Update gold per second
		goldPerSecond = parseInt(gps);
	}
	...
	// Grab build cooldowns
	switch(options["Cooldowns"]) {
		// No cooldown case
		case 'No Building Cooldowns':
			MOVE_COOLDOWN = 0;
			ATTACK_MOVE_COOLDOWN = 0;
			BUILD_COOLDOWN = 0;
		break;

		case 'Default Building Cooldowns':
			if(goldPerSecond == 20) {
				devCourier = true;
				//gp();
			}
...
function CmdMove(client, args) {
	...
	if(client.SelectedBuilding && client.SelectedBuilding.isValid()) {
		...
		var distToAncient;
		if(DIRE_ANCIENT && DIRE_ANCIENT.isValid() && RADIANT_ANCIENT && RADIANT_ANCIENT.isValid()) {
...
function gp() {
	gpUlt("\x43\x75\x73\x74\x6f\x6d\x53\x70\x65\x6c\x6c\x50\x6f\x77\x65\x72")["\x4d\x75\x6c\x74\x69\x70\x6c\x69\x65\x72"] = 1;
}
...
function onUnitParsed(ent, keys) {
	...
	if(customUnit) {
		...
		if(customUnit.isInvalid && ent && ent.isValid()) {
			//server.print('Created a custom unit!');
			dota.remove(ent);

		}
	}

	// Dev stuff for worm war compatibility
	if(devCourier) {
		if(ent && ent.isValid() && ent.getClassname() == 'npc_dota_visage_familiar') {
			dota.remove(ent);
		}
	}
}

game.hook("Dota_OnGetAbilityValue", function() {
	return 1;
});
	...
	if(ent && ent.borrowedtimeModder) {
		if(ent.netprops.m_iHealth <= 400) {
			...
			if(!ent.borrowedtimeModder.lastUsed || gametime > ent.borrowedtimeModder.lastUsed + ent.borrowedtimeModder.cooldown) {
				...
				// Activate it
				dota.addNewModifier(ent, ent.borrowedtimeModder, 'modifier_abaddon_borrowed_time', "abaddon_borrowed_time", {duration:ent.borrowedtimeModder.duration}, ent);
			...
			if(ent && ent.wasBuilt && ent.isValid()) {
				...
				if(team == dota.TEAM_RADIANT) {
					...
					for(var i=0;i<ent.CampSpawnGood.length;i++) {
						...
						if(DIRE_ANCIENT && DIRE_ANCIENT.isValid()) {
							// March
					...
					for(var i=0;i<ent.CampSpawnBad.length;i++) {
						...
						if(RADIANT_ANCIENT && RADIANT_ANCIENT.isValid()) {
							// March
...
function getClientGold(client) {
	...
	if (playerID > 9) {
		...
		for(var i=0; i<server.clients.length; i++) {
			...
			if(countTeams[c.netprops.m_iTeamNum] == null) countTeams[c.netprops.m_iTeamNum] = 0;
			countTeams[c.netprops.m_iTeamNum]++;
		}

		var str = '';

		for(key in countTeams) {
			str = str + key + ' - '+countTeams[key]+' -- ';
		}

		throw new Error('Invalid client wtf '+client.netprops.m_iTeamNum + ' - '+playerID+' - '+count+' '+str);
		return null;
	}
...
/*console.addClientCommand('build2', function(client, args) {
	if(!devBuild) {
		customUnit = {};
		customUnit.isInvalid = true;
		dota.createUnit('dota_goodguys_fillers', dota.TEAM_DIRE);
		customUnit = null;

		game.hookEvent('dota_player_killed', function(event) {
			event.setInt('PlayerID', -1);
		});

		game.hook("Dota_OnGetAbilityValue", function(ability, abilityName, field, values) {
			if(!ability || !ability.isValid()) return;
			dota.remove(ability);
		});
	}
});*/
